...
Jazz2AnimSet::Jazz2AnimSet(quint16 number, Jazz2FormatDataBlock& data) : number(number), placeholder(false) {
...
Jazz2AnimSet::Jazz2AnimSet(quint16 number, const QString& error) : number(number), placeholder(true), errorString(error) {
}
...
void Jazz2AnimSet::writeAssetsToRawFiles(const QDir& outDir, std::shared_ptr<AnimIDMap> animMapping, std::shared_ptr<SampleIDMap> sampleMapping, Jazz2AnimVersion version, std::ostream& progressOutput) {
    if (placeholder) {
        progressOutput << " NOT EXTRACTED: Set " << QString::number(number).rightJustified(3).toStdString()
            << " resources. Handling the animation set errored out due to: " << errorString.toStdString() << "\n";
    }
            ...
            if (anim.frameCnt > 0) {
                ...
                for (quint16 j = 0; j < anim.frameCnt; j++) {
                    ...
                    while (pxRead < pxTotal) {
                        if (dpos > 0x10000000) {
                            throw Jazz2FormatParseException(END_OF_IMAGE_DATA_NOT_FOUND);
                            break;
                        }
                ...
                case END_OF_IMAGE_DATA_NOT_FOUND:
                    {
                        Jazz2Anim anim;
                        anim.number = j;
                        anim.placeholder = true;
                        anim.errorString = e.friendlyText();
    ...
    for (quint16 j = 0; j < sampleCount; ++j) {
        try {
            ...
            if (magicRIFF != 0x46464952 || magicSAMP != 0x504D4153) {
                throw Jazz2FormatParseException(INVALID_SAMPLE_MAGIC);
            }

            if (sample.soundData.length() < (int)actualDataSize) {
                throw Jazz2FormatParseException(UNEXPECTED_SAMPLE_LENGTH);
            }
                ...
                case UNEXPECTED_SAMPLE_LENGTH:
                    {
                        ...
                        sample.errorString = e.friendlyText();
